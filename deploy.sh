...
# ============================================================
# Phase 5b: 安装 Karpenter v1.3.0
# ============================================================
echo ""
echo ">>> Phase 5b: 安装 Karpenter..."
...
# 给 private 子网和集群安全组打 Karpenter 发现标签
for subnet in $(echo $PRIVATE_SUBNETS | tr ',' ' '); do
...
# 创建 Karpenter Controller IAM 角色 (IRSA)
...
aws iam create-role --role-name KarpenterControllerRole-$CLUSTER_NAME \
  --assume-role-policy-document file:///tmp/karpenter-trust.json 2>/dev/null || true

cat > /tmp/karpenter-controller-policy.json <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {"Sid": "Karpenter", "Effect": "Allow", "Action": [
    ...
    {"Sid": "TerminateTagged", "Effect": "Allow", "Action": "ec2:TerminateInstances", "Resource": "*",
    ...
    {"Sid": "EKS", "Effect": "Allow", "Action": "eks:DescribeCluster",
     "Resource": "arn:aws:eks:${REGION}:${ACCOUNT_ID}:cluster/${CLUSTER_NAME}"},
    {"Sid": "InstanceProfile", "Effect": "Allow", "Action": [
...
# ============================================================
# Phase 7b: 创建 WAFv2 WebACL 并关联 ALB
# ============================================================
echo ""
echo ">>> Phase 7b: 创建 WAFv2 WebACL..."
WAF_ARN=$(aws wafv2 create-web-acl --name litellm-waf --scope REGIONAL \
  ...
  --tags Key=Project,Value=litellm --region $REGION \
  --query "Summary.ARN" --output text 2>/dev/null)

if [ -n "$WAF_ARN" ]; then
  echo "  ✅ WAF WebACL: $WAF_ARN"
  echo "  ⚠️  请将 WAF ARN 更新到 07-ingress.yaml 的 wafv2-acl-arn 注解后重新 apply"
else
  echo "  ⚠️  WAF 可能已存在，请手动确认并关联到 ALB"
fi
